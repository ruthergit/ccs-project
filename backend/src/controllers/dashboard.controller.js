...
export const getDashboardStats = async (_req, res) => {
  try {
    ...
    const [[{ upcomingEvents}]] = await pool.query("SELECT COUNT(*) AS upcomingEvents FROM events WHERE status = 'Upcoming'");
    const [[{ totalSchedules}]] = await pool.query('SELECT COUNT(*) AS totalSchedules FROM schedules');
    ...
    res.json({
      totalStudents, totalFaculty, upcomingEvents, totalSchedules,
      topSkill,
